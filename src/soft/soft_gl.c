...
bool chk_soft_gl_init(SoftGL *gl, int win_w, int win_h)
{
    if (!gl)
    {
        fprintf(stderr, "chk_soft_gl_init: gl was NULL.\n");
        return false;
    }
...
void chk_soft_gl_update_tex(SoftGL *gl, Bitmap *bmp)
{
    ...
    int format = GL_RGBA;
    if (bmp->bpp != 4)
        fprintf(stderr, "chk_soft_gl_update_tex: Unsupported bitmap bpp of %d!", bmp->bpp);
...
{
    if (!gl->shader_program)
        fprintf(stderr, "chk_soft_gl_present: Missing shader program!");
